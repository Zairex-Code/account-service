...
public class GetAccountUseCaseImpl implements GetAccountUseCase {
     ...
     * @throws IllegalArgumentException if no account is found with the given identifier
     */
    @Override
    public Single<Account> findById(String id) {
        ...
        return accountPersistencePort.findById(id)
                .switchIfEmpty(Single.error(new IllegalArgumentException(
                        "Cannot retrieve account: Account with ID '" + id + "' was not found")))
    ...
    public Maybe<Account> findByAccountNumber(String accountNumber) {
        ...
        return accountPersistencePort.findByAccountNumber(accountNumber)
                .doOnSuccess(account -> log.debug("Account found with Account Number: {}", accountNumber));
    }
